...
3. Pull the underlying cases for the top driver:
   ```sql
   select case_id, customer_id, issue, resolved, revenue_at_risk, csat_score
   from fct_case_enriched
   where root_cause = :top_root_cause
   order by revenue_at_risk desc;
   ```
4. Explain in 2-3 sentences: the driver, the dollars at risk, how many cases / how many
